 ...
 * 添加我的资源或编辑我的资源
 */
public class FragSupplyEdit extends FragBase {
	...
	public static void invoke(Context context, Resource resource) {
		...
		param.clsFrag = FragSupplyEdit.class;
		...
		intent.putExtra(INTENT_KEY_SUPPLY, resource);
	...
	static TitleRunnable titleRunnable = new TitleRunnable() {
		...
		protected void execute(Context context, Fragment fragment) {
			if (tagId == TAG_ID_RIGHT) {
				if (fragment != null && fragment instanceof FragSupplyEdit) {
					((FragSupplyEdit) fragment).saveSupply();
				}
	...
	protected String getPageName() {
        if (resource == null){
            return "ProfileSupplyCreate";
        }else {
            return "ProfileSupplyEdit";
        }
	}

	public void initViews() {
		tvTitle.setText("我愿为岛亲提供的资源");
		tvContentTitle.setText("资源介绍");
		etContent.setHint("请输入资源详情");
		tvDelete.setText("删除该资源");
