...
public class FragEventDetail extends FragBase {
    ...
    @InjectView(R.id.tvPriceDiscount)
    ...
    RelativeLayout llPriceGuests;

    @InjectView(R.id.llPriceVip)
    RelativeLayout llPriceVip;

    @InjectView(R.id.llPriceDiscount)
    ...
    @InjectView(R.id.iv_PriceGuests_Coupon)
    ImageView ivPriceGuestsCoupon;

    @InjectView(R.id.tvPriceGuestsCoupon)
    TextView tvPriceGuestsCoupon;

    @InjectView(R.id.iv_PriceVipCoupon)
    ImageView ivPriceVipCoupon;

    @InjectView(R.id.tvCouponPriceVip)
    TextView tvCouponPriceVip;
    ...
    private void setOfficialPriceColor() {
        ...
        if (user.isVip() && !TextUtils.isEmpty(event.vipPriceText)) {
            ...
            tvPriceGuestsCoupon.setTextColor(colorF2);
        } else {
            // app不存在原价用户和受邀用户价的用户
            // 没有对应身份的价格 ，都不高亮。
        }
...
//        String tvPriceName;
//        String tvPriceCouponName;
//
//        if (isVip) {
//            tvPriceName = "岛邻价";
//            tvPriceCouponName = "岛邻优惠价";
//        } else {
//            tvPriceName = "海客价";
//            tvPriceCouponName = "海客优惠价";
//        }
//
//        //有优惠价格
//        if (price.contains("</s>")) {
...
//            tvPriceCoupon.setVisibility(View.VISIBLE);
//            tvPrice.setVisibility(View.VISIBLE);
//
//            tvName.setText(tvPriceCouponName);
//            String arr[] = price.split("</s>");
//            String couponPrice = arr[0].substring(3, arr[0].length());
//            String tPrice = arr[1];
//
//            tvPriceCoupon.setText(couponPrice);
//            tvPriceCoupon.getPaint().setAntiAlias(true);//抗锯齿
//            tvPriceCoupon.getPaint().setFlags(Paint.STRIKE_THRU_TEXT_FLAG | Paint.ANTI_ALIAS_FLAG);  // 设置中划线并加清晰
//            tvPrice.setText(tPrice);
//
//        } else {
        //tvPrice.setVisibility(View.VISIBLE);
//            tvName.setText(tvPriceName);
