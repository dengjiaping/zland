...
public class UserDeprecatedDao extends BaseDaoImpl<UserDeprecated, Long> {
	...
	public void updateUserNotNull(UserDeprecated user) {
		if (user == null) {
			return;
		}
		QueryBuilder<UserDeprecated, Long> qb = this.queryBuilder();
		try {
			List<UserDeprecated> query = qb.where()
					.eq(UserDeprecated.COL_ID, user.uid).query();
			if (query != null && query.size() > 0) {
				UpdateBuilder<UserDeprecated, Long> ub = this.updateBuilder();
				ub.where().eq(UserDeprecated.COL_ID, user.uid);
				DBUtil.addColumnNotNull(ub, user);
				ub.update();
			} else {
				this.create(user);
			}
	...
	public HashMap<String, UserDeprecated> getIconIdMap(
			ArrayList<ContactGroup> contactGroups) {
		HashMap<String, UserDeprecated> jidUserMap = new HashMap<String, UserDeprecated>();
	...
	public void addIconIdMap(HashMap<String, UserDeprecated> jidUserMap,
			List<IMContact> data) {
	...
	public void addIconIdMapOfChat(HashMap<String, UserDeprecated> jidUserMap,
			List<IMChat> chats) {
