	...
	private final OnLongClickListener longClickListener = new OnLongClickListener() {
		...
		public boolean onLongClick(View v) {
			if (adapterListener != null) {
				adapterListener.onUserAt(userOther.getName());
			}
			return true;
		}
	};
	...
	public void onClick(View view) {
		...
		case R.id.chat_row_avator:
			if(msg.isSendBySelf()){
				...
				ActProfileDetail.invoke(context, IMContact.parseUid(msg.contact));
			}
			break;
		...
		if (msg.direction == Constants.MSG_OUTGOING) {
			...
			if (userSelf != null) {
				ImageWorkFactory.getCircleFetcher().loadImage(
						userSelf.getAvatar_M_Url(), ivAvator,
			...
			if (userOther != null) {
				ImageWorkFactory.getCircleFetcher().loadImage(
						userOther.getAvatar_M_Url(), ivAvator,
						R.drawable.avatar_default);
				// if (msg.groupMessage) {
