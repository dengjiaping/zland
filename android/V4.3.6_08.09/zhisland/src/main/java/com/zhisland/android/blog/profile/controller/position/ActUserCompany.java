...
public class ActUserCompany extends FragBaseActivity {
	...
	public static final String INK_USER_IS_SELF = "ink_user_is_self";
	
	FragUserCompany fragProfileJob;

	public static void invoke(Context context, ArrayList<Company> userPositionList, boolean isSelf) {
		Intent intent = new Intent(context, ActUserCompany.class);
		intent.putExtra(INK_USER_COMPANY, userPositionList);
		intent.putExtra(INK_USER_IS_SELF, isSelf);
