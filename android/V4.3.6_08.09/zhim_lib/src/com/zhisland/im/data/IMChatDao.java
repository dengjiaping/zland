...
public class IMChatDao extends IMBaseDao<IMChat, Long> {
	...
	public void fetchContacts(List<Contact> openedChats) {
		QueryBuilder<IMChat, Long> qb = queryBuilder();
		try {
			List<IMChat> chats = qb.query();
			Contact contact;
			if (chats != null) {
				for (IMChat chat : chats) {
					contact = new Contact(chat.contact);
					contact.setName(chat.name);
					contact.setAvatarId(chat.avatar);
					contact.setMessage(chat.message);
					contact.setType(chat.type);
					contact.setTimestamp(chat.time);
					contact.setUnRead(chat.unread);
					openedChats.add(contact);
				}
			}
		} catch (Exception e) {
			Log.d("db", e.getMessage(), e);
