			...
			if (block.type == SimpleBlock.TYPE_USER_COMMENT) {
				...
				if (block.type == SimpleBlock.TYPE_ASSISTANT) {
					...
					if (assistant.visibleRange == null) {
						assistant.visibleRange = UserAssistant.VALUE_CONTACT_VISIBLE_RANGE_VIP;
					}
					if (assistant.visibleRange == UserAssistant.VALUE_CONTACT_VISIBLE_RANGE_VIP) {
						if (!DBMgr.getMgr().getUserDao().getSelfUser().isVip()) {
							return false;
						}
					} else if (assistant.visibleRange == UserAssistant.VALUE_CONTACT_VISIBLE_RANGE_FRIEND) {
						if (!com.zhisland.im.data.DBMgr.getHelper()
					...
					if (contactInfo.visibleRange == null) {
						contactInfo.visibleRange = UserContactInfo.VALUE_CONTACT_VISIBLE_RANGE_VIP;
					}
					if (contactInfo.visibleRange == UserContactInfo.VALUE_CONTACT_VISIBLE_RANGE_VIP) {
						if (!DBMgr.getMgr().getUserDao().getSelfUser().isVip()) {
							return false;
						}
		...
		if (blocks != null) {
			for (SimpleBlock block : blocks) {
				...
				if (blockView != null) {
					...
					llCoverContainer.addView(getBlockDividerView());
				}
			}
		}
	}
