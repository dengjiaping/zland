...
public class ImSessAdapter extends BaseListAutoSectionAdapter<IMMessage>
	...
	private Contact userSelf;
	private Contact userOther;
	...
	public ImSessAdapter(Context context) {
		...
		if (userSelf == null) {
			...
			String jid = IMContact.buildJid(PrefUtil.Instance().getUserId());
			userSelf = new Contact(jid);
			userSelf.setName(name);
			userSelf.setAvatarId(avatar);
		}
	}
	 ...
	 * set other user info
	 */
	public void setUser(Contact mUser) {
			...
			numberDialog = DialogUtil.createActionSheet(context, title, "取消",
					menus, new OnActionClick() {
						...
						public void onClick(DialogInterface dialog, int which,
							...
							case 1: {
								IMLinkMovementMethod.onNumberClicked(
										strLinkMenu, context);
								numberDialog.dismiss();
								break;
							}
