...
public class DBUpdateUtil {
	...
	private static DBUpdateUtil dbUpdateUtil;
	...
	public void update(SQLiteDatabase db, ConnectionSource connection) {
		...
		DBMgr.getMgr().createTable(connection);
	...
	static {
		// 加入要创建的表名
		allTableList.add(UserDeprecated.TB_NAME);
		allTableList.add(User.TB_NAME);
		allTableList.add(CacheDto.TB_NAME);
		allTableList.add(Dict.TB_NAME);
		// 加入升级时要创建的临时表表名
		allTableList.add("temp_" + UserDeprecated.TB_NAME);
		allTableList.add("temp_" + User.TB_NAME);
		allTableList.add("temp_" + CacheDto.TB_NAME);
		allTableList.add("temp_" + Dict.TB_NAME);
	}
}
