 ...
 * 擅长领域
 */
public class FragProfileSpecialty extends FragBase {
	...
	protected String getPageName() {
		return "fragProfileSpecialty";
	}

	@OnClick(R.id.btnNext)
	public void onClickNext() {
		((ActSelTags) getActivity()).showWantFriends();
	}

	public void initViews() {
		tags.clear();
		tags.addAll(LoadDataFromAssert.getInstance().loadSpecialtieTags());
		addTags();
	...
	private void fillUser() {
		...
		if (user != null && specialtiesTags.size() > 0) {
			// 设置默认值
			selTags.addAll(specialtiesTags);
