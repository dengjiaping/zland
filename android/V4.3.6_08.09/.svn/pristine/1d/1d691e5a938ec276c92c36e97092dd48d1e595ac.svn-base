...
public class TagFlowLayout extends FlowLayout implements
	...
	/**
	 * 设置当已经选择了限制数量后，再点击其他的选择，弹出的提示。为空不提示。
	 * */
	...
	private void doSelect(TagView child, int position) {
		if (mAutoSelectEffect) {
			if (!child.isChecked()) {
				...
				// }
				if (mSelectedMax == 0 || mSelectedView.size() == mSelectedMax) {
					if (!StringUtil.isNullOrEmpty(toastContent)) {
						ToastUtil.showShort(toastContent);
					}
				} else if (mSelectedMax > 0
						&& mSelectedView.size() > mSelectedMax) {
					int i = 0;
					for (Integer index : mSelectedView) {
						if (i >= mSelectedMax) {
							TagView deleteTag = (TagView) getChildAt(index);
							deleteTag.setChecked(false);
