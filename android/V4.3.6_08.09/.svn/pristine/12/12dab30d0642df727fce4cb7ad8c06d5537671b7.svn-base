...
public class User extends OrmDto {
    ...
    private static final int VALUE_TYPE_YUZHUCE = 50; // 预注册
	...
	public String getUserTypeDesc() {
		if (userType != null && userType == VALUE_TYPE_VIP) {
			...
			return "海客";
		} else if (userType != null && userType == VALUE_TYPE_YUZHUCE) {
			return "访客";
		}
		return "";
	}
    ...
    public boolean isYuZhuCe(){
        if (userType != null && userType ==  VALUE_TYPE_YUZHUCE){
            return true;
        }
        return false;
    }
	 ...
	 * */
	public SpannableString getNameWithTypeGoldFire(Context context) {
		int margin = DensityUtil.dip2px(5);
		String nameStr = name == null ? "" : name;
		ImageSpan goldFireSpan = null;
		ImageSpan typeSpan = null;
		int drawableId = getVipIconId();
		if (drawableId != -1) {
			nameStr += "占";
