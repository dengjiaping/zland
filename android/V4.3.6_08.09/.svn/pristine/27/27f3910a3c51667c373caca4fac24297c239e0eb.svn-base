...
public abstract class TabWrapper implements TabBarViewListener,
	...
	public static final int PROFILE_COVER = 1;
	public static final int PROFILE_FRIEND_IMPRESSION = 2;
	...
	private void setTabs(TabBarView tabBarView) {
		...
		ZHTabInfo tab = new ZHTabInfo("   个人封面    ", PROFILE_COVER);
		tabs.add(tab);
		tab = new ZHTabInfo("朋友眼中的我", PROFILE_FRIEND_IMPRESSION);
		tabs.add(tab);
		tabBarView.setTabs(tabs);
	}
	
	public void refreshBtnName(boolean isSelf, boolean isMan) {
		for (TabButton tabButton : tabButtonList) {
			if (tabButton.tabId == PROFILE_FRIEND_IMPRESSION) {
				if(isSelf){
					tabButton.text.setText("朋友眼中的我");
				}else{
					if(isMan){
						tabButton.text.setText("朋友眼中的他");	
					}else{
						tabButton.text.setText("朋友眼中的她");
					}
				}
			}
		}
