...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public final void onRefreshComplete() {
		Log.d(LOG_TAG, "onRefreshComplete " + isRefreshing());
		if (isRefreshing()) {
	...
	protected void onRefreshing(final boolean doScroll) {
		Log.d(LOG_TAG, "refresh with " + doScroll);
	...
	/**
	 * can override this methods to use custom wrapper
	 * 
	 * @param context
	...
	private void addRefreshableView(Context context, T refreshableView) {
		...
		// ViewGroup.LayoutParams.MATCH_PARENT,
		// ViewGroup.LayoutParams.MATCH_PARENT);
		mRefreshableViewWrapper = createRefreshWrapper(context, refreshableView);
	...
	@SuppressWarnings("deprecation") //过期的方法不处理
	...
	private final void smoothScrollTo(int newScrollValue, long duration,
		...
		case VERTICAL:
		default:
			oldScrollValue = getMyScrollY();
			break;
		}
		
		Log.d(LOG_TAG, "smoothScrollTo " + oldScrollValue + "   "
				+ newScrollValue);
