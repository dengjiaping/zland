...
public class ShareDialog extends Dialog {
    ...
    public static final int ID_TRANSPARENT = -100;//id是这个的
    ...
    public ShareDialog(Context context, int theme,
        ...
        this.adjust(actionItems, numColumns);
    }

    //适配下多上少的布局
    private void adjust(List<ActionItem> actionItems, int numColumns) {
        if (actionItems == null || actionItems.size() < 1) {
            return;
        }
        int size = actionItems.size();
        int unFitNum = size % numColumns;
        if (unFitNum == 0) {
            //正好能够排满N行，不需要失陪
            return;
        }

        for (int i = unFitNum; i < numColumns; i++) {
            ActionItem item = new ActionItem(ID_TRANSPARENT, null);
            actionItems.add(i, item);
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gvShare.setVerticalSpacing(DensityUtil.dip2px(10));
    ...
    public static class Holder implements View.OnClickListener {
        ...
        public void fill(ActionItem item) {
            this.item = item;
            if (item.id != ID_TRANSPARENT) {
                ivShare.setImageResource(item.resId);
                tvShare.setText(item.name);
            } else {
                ivShare.setImageBitmap(null);
                tvShare.setText(null);
            }
        }

        @Override
        public void onClick(View v) {
            if (actionClick != null && item.id != ID_TRANSPARENT) {
