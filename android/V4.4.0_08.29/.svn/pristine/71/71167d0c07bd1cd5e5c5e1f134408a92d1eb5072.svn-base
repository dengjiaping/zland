...
public class FragUserResource extends FragBase {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (userDetail != null && userDetail instanceof UserDetail) {
            getDataFromUserDetail((UserDetail) userDetail);
        } else {
            getUserDetail();
        }
    }
    ...
    private void getUserDetail() {
        Object obj = DBMgr.getMgr().getCacheDao().get(UserDetail.CACH_USER_DETAIL + PrefUtil.Instance().getUserId());
        if (obj != null && obj instanceof UserDetail) {
            getDataFromUserDetail((UserDetail) obj);
        }
    ...
    private void getDataFromUserDetail(UserDetail content) {
        if (demandblock == null) {
            demandblock = new SimpleBlock<Resource>();
            demandblock.type = SimpleBlock.TYPE_DEMAND;
            demandblock.data = new ArrayList<>();
        }
        if (supplyblock == null) {
            supplyblock = new SimpleBlock<Resource>();
            supplyblock.type = SimpleBlock.TYPE_SUPPLY;
            supplyblock.data = new ArrayList<>();
        }
        if (content != null) {
            demandblock.data.clear();
            supplyblock.data.clear();
            SimpleBlock<Resource> block = content.getResourceBlock();
            if (block != null && block.data != null) {
                for (Resource resource : block.data) {
                    if (resource.type == Resource.TYPE_DEMAND) {
                        demandblock.data.add(resource);
                    } else {
                        supplyblock.data.add(resource);
                    }
                }
                demandblock.maxCount = block.maxCount;
                supplyblock.maxCount = block.maxCount;
            }
        }
    ...
    void addSupplyClick() {
        if (PermissionsMgr.getInstance().canResourceAndDemandPublish()) {
            if (supplyblock != null && supplyblock.maxCount != null
                ...
                showToast("您现在只能发布" + supplyblock.maxCount + "条资源");
                return;
            }
    ...
    void addDemandClick() {
        if (PermissionsMgr.getInstance().canResourceAndDemandPublish()) {
            if (demandblock != null && demandblock.maxCount != null
                ...
                showToast("您现在只能发布" + demandblock.maxCount + "条需求");
                return;
            }
