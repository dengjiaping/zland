...
public class BeemOtrManager implements OtrEngineHost {
	...
	private BeemOtrManager() {
		...
		try {
			mOtrKeyManager = new OtrKeyManagerImpl(
	...
	/**
	 * We must call addChat before stating a new otr session because we will
	 * need the chat instance for message injection.
	 *
	 * @param sessionID
	 *            the otr sessionID.
	 * @param chat
	...
	/**
	 * We must remove the chat from the map after we ended the corresponding otr
	 * session.
	 *
	 * @param sessionID
