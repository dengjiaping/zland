...
public class FragContactList extends
        FragPullSectionList<UserGroup, IMUser> {
    ...
    ArrayList<IMUser> searchResultData = new ArrayList<IMUser>();
    ...
    public void onEventMainThread(IMUser contact) {
        if (contact.action == IMBase.UPDATE) {
            if (UserGroups != null) {
                UserGroup desGroup = null;
                for (UserGroup cg : UserGroups) {
                ...
                if (desGroup != null) {
                    ArrayList<IMUser> contacts = desGroup.contacts;
                    if (contacts != null) {
                        IMUser desContact = null;
                        int index = 0;
                        ...
                        if (desContact != null) {
                            ...
                            this.contactAdapter.setGroup(UserGroups);
    ...
    private void loadContact() {
        UserGroups = getDataFromDB();
        jidUserMap = com.zhisland.android.blog.common.dto.DBMgr.getMgr().getUserDao()
                .getIconIdMap(UserGroups);
        getPullProxy().onLoadSucessfully(UserGroups);
        loadLetters();
    ...
    private ArrayList<UserGroup> getDataFromDB() {
        return DBMgr.getHelper().getUserDao().fetchContactGroups(
                RelationConstants.BOTH_FLLOWED + "", IMUser.COL_CHAR, true);
    }

    private void loadLetters() {
        char[] chars = new char[UserGroups.size()];
    ...
    OnLetterSelectedListener onLetterSelectedListener = new OnLetterSelectedListener() {
        ...
        public void onLetterSelected(char paramChar) {
            ArrayList<UserGroup> sections = contactAdapter.getGroups();
            if (sections == null) {
                return;
            }
            ...
            for (UserGroup section : sections) {
    ...
    class SearchHolder {
        ...
        public void fill(IMUser contact, boolean showLine) {
    ...
    private ArrayList<IMUser> getSearchResult(String keyword) {
        ArrayList<IMUser> nameList = new ArrayList<IMUser>();
        ArrayList<IMUser> descList = new ArrayList<IMUser>();
        ArrayList<IMUser> result = new ArrayList<IMUser>();

        for (UserGroup UserGroup : UserGroups) {
            for (IMUser contact : UserGroup.contacts) {
    ...
    class ContactAdapter extends
            BaseSectionListAdapter<UserGroup, IMUser> {
        ...
        public View getRealChildView(int groupPosition, int childPosition,
            ...
            IMUser item = getChild(groupPosition, childPosition);
