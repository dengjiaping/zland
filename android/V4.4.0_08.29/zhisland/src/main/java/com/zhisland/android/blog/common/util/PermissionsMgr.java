...
import com.zhisland.android.blog.common.api.TaskGetPermissions;
...
public class PermissionsMgr {
     ...
     * 资讯推荐文章
     */
    public boolean canPublicFeed() {
        //TODO use server config
//        List<String> permissions = getPermissions();
//        String path = STR_SEP + AUriMgr.PATH_FEED_ADD;
//        if (permissions == null) {
        // 没拉取到数据的默认值
        if (isYuZhuCe()) {
            return false;
...
//        }
    ...
    private boolean isHaiKe() {
        ...
        return selfUser != null ? selfUser.isHaiKe() : false;
    }
    ...
    private boolean isYuZhuCe() {
        User selfUser = DBMgr.getMgr().getUserDao().getSelfUser();
        return selfUser != null ? selfUser.isYuZhuCe() : false;
    }
    ...
    private boolean isVip() {
        User selfUser = DBMgr.getMgr().getUserDao().getSelfUser();
        return selfUser != null ? selfUser.isVip() : false;
    }
    ...
    private boolean isDaoDing() {
        User selfUser = DBMgr.getMgr().getUserDao().getSelfUser();
        return selfUser != null ? selfUser.isDaoDing() : false;
    }
}
