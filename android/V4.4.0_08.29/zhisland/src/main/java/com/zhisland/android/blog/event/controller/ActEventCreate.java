...
public class ActEventCreate extends FragBaseActivity {
    ...
    public static final int FROM_CREATE_SHARE = 2;
     ...
     * 启动活动编辑页面，创建活动
     */
    public static void invoke(Context context, Event event, int from) {
        Intent intent = getInvokeIntent(context, event, from);
    ...
    public static void invokeForResult(Activity context, Event event,
                                       int reqCode, int from) {
        Intent intent = getInvokeIntent(context, event, from);
        context.startActivityForResult(intent, reqCode);
    }

    private static Intent getInvokeIntent(Context context, Event event, int from) {
        Intent intent = new Intent(context, ActEventCreate.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable(KEY_EVENT, event);
        if (event != null) {
            bundle.putInt(KEY_FROM, FROM_MODIFY_EVENT);
        } else if (from >= 0) {
            bundle.putInt(KEY_FROM, from);
        } else {
            bundle.putInt(KEY_FROM, FROM_CREATE_EVENT);
        }
    ...
    public void onBackPressed() {
        if (fragEventCreateSecond == null) {
            if (from == FROM_CREATE_EVENT || from == FROM_CREATE_SHARE) {
                cancelCreate();
                ...
                public void onClick(View v) {
                    if (!isCancel) {
                        fragEventCreateSecond
                                .createOrUpdateEvent(from != FROM_MODIFY_EVENT);
                    } else {
                        finish();
                    }
                    dialog.dismiss();
    ...
    public void showSecondStep() {
        if (fragEventCreateSecond == null) {
            ...
            fragEventCreateSecond.setShare(from == FROM_CREATE_SHARE);
        }
