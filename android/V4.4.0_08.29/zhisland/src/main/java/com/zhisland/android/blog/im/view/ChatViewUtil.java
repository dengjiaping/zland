...
public class ChatViewUtil {
    ...
    public static ChatViewUtil instance() {
        if (_instance == null) {
            synchronized (lockObj) {
                if (_instance == null) {
                    _instance = new ChatViewUtil();
                }
            }
        }
        return _instance;
    ...
    private ChatViewUtil() {
        parser = ExpressParser.getInstance();
    ...
    public CharSequence formatText(Context context, String text, OnLinkClickListener listener,
                                   int textHeight) {
        ...
        CharSequence seq = Html.fromHtml(text, parser.expIconGetter(context), null);
